...
final class SQLGenerator
{
    ...
    private static function _faculty( $dept, $parent_dept ) : string
    {
        ...
        $sql = "
            SELECT DISTINCT u.id,
                            ...
                            u.email,
                            u.phone,
                            u.photo_path                                        AS photo,
                            u.photo_extra,
                            u.interests,
                            t.id                                                AS title_id,
                            Concat_ws(' ', t.description, ud.title_description) AS title,
                            t.description                                       AS hr_title,
                            ud.prog_title_dept                                  AS
                            prog_title,
                            ud.prog_title_dept_short                            AS
                            title_short,
                            ud.affiliation,
                            t.title_group,
                            ac.id                                               AS subdept,
                            ...
                            r.building
                    FROM   users AS u
                        LEFT JOIN users_departments AS ud
                               ON u.id = ud.user_id
                        LEFT JOIN academics AS a
                               ON a.user_id = u.id
                        LEFT JOIN academic_categories AS ac
                               ON ac.id = a.academic_id
                        LEFT JOIN titles AS t
                               ON t.id = ud.title_id
                        LEFT JOIN (SELECT rooms.id,
                                          rooms.room_number           AS room,
                                          buildings.short_description AS `desc`,
                                          buildings.building_number   AS building
                                   FROM   rooms
                                          LEFT JOIN buildings
                                                 ON rooms.building_id = buildings.id) AS r
                               ON r.id = u.room_id
                    WHERE  ud.department_id IN ( $parent_dept )
                           AND ( a.academic_id IN( $dept, 74, 75, 79, 81, 82 )
                              OR t.id IN( 10, 102 ) )
                           AND ( ud.affiliation IN( 'active', 'emeritus' ) )
        ";

        //$sql = "SELECT DISTINCT u.id, u.lname, u.fname, u.mname, u.email, u.phone, u.photo_path AS photo, u.photo_extra, u.interests, t.id AS title_id, CONCAT_WS( ' ', t.description, ud.title_description) AS title, t.description AS hr_title, ud.prog_title_dept AS prog_title, ud.prog_title_dept_short AS title_short, ud.affiliation, t.title_group, ac.id AS subdept, a.ordinal, u.activities, u.awards, u.research, u.has_cv, u.homepage, u.biography AS bio, u.office, r.room, r.`desc` AS room_desc, r.building FROM users AS u LEFT JOIN users_departments AS ud ON u.id = ud.user_id LEFT JOIN academics AS a ON a.user_id = u.id LEFT JOIN academic_categories AS ac ON ac.id = a.academic_id LEFT JOIN titles AS t ON t.id = ud.title_id LEFT JOIN ( SELECT rooms.id, rooms.room_number AS room, buildings.short_description AS `desc`, buildings.building_number AS building FROM rooms LEFT JOIN buildings ON rooms.building_id = buildings.id  ) AS r ON r.id = u.room_id WHERE ud.department_id IN ( $parent_dept ) AND ( a.academic_id IN( $dept, 74, 75, 79, 81 ) OR t.id IN( 10, 102 ) ) AND ( ud.affiliation IN( 'active', 'emeritus' ) )";
        ...
        else
        {
            $sql = "
                ...
                WHERE    department_id IN($dept)
                         OR id IN(74, 75, 79, 81, 82)
                ORDER BY department_id,
                         `description`
            ";

            //$sql = "SELECT id, description, department_id FROM academic_categories WHERE department_id IN( $dept ) OR id IN( 74, 75, 79, 81 ) ORDER BY department_id, description";
        }
    ...
    private static function _edu( $id ) : string
    {
        $sql = "
            SELECT      education.field,
                        ...
                        education.year                  AS degYear,
                        `degrees`.short_description     AS `degree`
            FROM        cah.education
                        LEFT JOIN   cah.degrees
                                    ON  education.degrees_id = `degrees`.id
            WHERE       education.user_id = $id
            ORDER BY    education.year
        ";
    ...
    {
        $sql = "
            SELECT      pubs.forthcoming,
                        DATE_FORMAT(pubs.publish_date, '%M %Y')             AS pubDate,
                        pub_cats.plural_description                         AS pubType,
                        pubs.citation
            FROM        cah.publications                                    AS pubs
                        LEFT JOIN   cah.publications_categories             AS pub_cats
                                    ON pubs.publication_id = pub_cats.id
            WHERE       pubs.user_id = $id
                        AND approved = 1
            ORDER BY    pub_cats.level,
                        pub_cats.plural_description,
                        pubs.publish_date DESC
        ";
    ...
    private static function _courses( $id, $term, $aux ) : string
    {
        $sql = "
            SELECT      courses.id,
                        `number`,
                        IF(`description` IS NULL, 'No Description Available', `description`)    AS `description`,
                        CONCAT(prefix, catalog_number)                                          AS catalogref,
                        syllabus_file,
                        term,
                        section,
                        title,
    ...
    private static function _terms() : string
    {
        $sqlStart = "
            SELECT DISTINCT term,
                            CAST( SUBSTRING(term, LOCATE(' ', term)) AS UNSIGNED)
                            + CAST( IF( SUBSTRING_INDEX(term, ' ', 1) = 'Fall', 1, 0) AS UNSIGNED)  AS ordering
            FROM            courses
            WHERE           term != CONCAT('Summer', (YEAR( NOW() ) + 1))
        ";
        ...
        return $sqlStart . $sqlEnd . " LIMIT 0, 5";
